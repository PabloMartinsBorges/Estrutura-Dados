...
public class ConjGenerico<T extends Comparable<T>>
{
	...
	public ConjGenerico<T> complementar(ConjGenerico<T> universo){
		...
		for(p = universo.prim; (p!= null); p = p.prox) {
			...
			if(p.dado.compareTo(q.dado)> 0)
				throw new RuntimeException("Possui elemento " + q.dado + " fora do Conjunto universo");
			else if(p.dado.compareTo(q.dado) == 0)
				q = q.prox;
			else if(p.dado.compareTo(q.dado) < 0 && p.prox == null)
				throw new RuntimeException("Possui elemento " + q.dado + " fora do Conjunto universo");
